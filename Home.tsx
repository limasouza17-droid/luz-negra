...
function Home() {
  ...
  return <main className="site-shell">
    ...
    <section className="section experiment-section">
      ...
      <div className="experiment-grid"><article><span className="mono">HIPÓTESE</span><h3>Quais materiais apresentarão fluorescência sob luz UV?</h3><div className="line-fill" /></article><article><span className="mono">PROCEDIMENTO</span><ol><li>Observar sob luz branca.</li><li>Expor à luz UV.</li><li>Registrar as diferenças.</li><li>Comparar os resultados.</li></ol></article><article className="photo-placeholder"><CameraFrame /><span>adicione aqui as fotos reais</span><small>área reservada para evidência</small></article></div>
    </section>

    <section className="section results-section"><SectionMarker number="10" label="Resultados · folha de leitura" /><div className="split-heading"><h2>Dados que<br /><i>contam a história.</i></h2><p>Preencha somente com os resultados reais de vocês. A investigação fica mais forte quando cada brilho tem uma evidência.</p></div><div className="result-table"><div className="table-row table-head"><span>material</span><span>luz branca</span><span>luz UV</span><span>resultado</span></div>{objects.slice(0, 3).map((o, i) => <div className="table-row" key={o.name}><b>{o.name}</b><span>◉</span><span className={i === 0 ? "spark" : ""}>{i === 0 ? "✦" : "◉"}</span><strong>{i === 0 ? "fluorescência" : i === 1 ? "pouca" : "a testar"}</strong></div>)}</div></section>
